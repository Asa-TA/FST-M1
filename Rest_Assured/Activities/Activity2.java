...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "testapi") // Pass request body from file
        ...
        response.then().body("username", equalTo("testapi"));
        response.then().body("firstName", equalTo("api"));
        response.then().body("lastName", equalTo("test"));
        response.then().body("email", equalTo("testcase@mail.com"));
    ...
    public void deleteUser() throws IOException {
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "testapi") // Add path parameter
            .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("testapi"));
    }
}